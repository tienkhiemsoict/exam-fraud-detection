...
class Landmark:
    ...
    label: str          # Nhãn mô tả (vd: "A1", "B2", etc.)
    
    def __hash__(self) -> int:
        """Cho phép sử dụng Landmark trong Set và Dict"""
        return hash((self.id, self.position, self.label))
    
    def __eq__(self, other) -> bool:
        """So sánh hai điểm mốc"""
...
class ExamDetector:
    """Hệ thống phát hiện người trong phòng thi"""
    ...
    def __init__(self, 
                 ...
                 stability_threshold: int = 3,        # Số lần detect liên tiếp để xác nhận vị trí ổn định
    ...
    def detect_persons(self, frame: np.ndarray) -> List[DetectedPerson]:
        ...
        current_track_positions = {i: p.nearest_landmark for i, p in enumerate(detected_people) if p.nearest_landmark}
    ...
    def draw_results(self, frame: np.ndarray, detected_people: List[DetectedPerson]) -> np.ndarray:
        ...
        for i, person in enumerate(detected_people):
            ...
            if person.nearest_landmark:
                ...
                # Vẽ text thông tin
                text = f"ID:{i} - Pos:{person.nearest_landmark.label}"
                if person.stable_position:
                    text += " (Stable)"
    ...
    def process_video(self, video_path: str, output_path: str = None,
        ...
        try:
            ...
            while frame_count < total_frames and should_continue:
                ...
                frame_with_results = self.draw_results(frame, detected_people)
                ...
                if display:
                    ...
                    window_name = 'Exam Detection (Front Camera)'
                    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
                    
                    # Resize frame và hiển thị
                    display_frame = cv2.resize(frame_with_results, (display_w, display_h), 
                ...
                if not should_continue:
                    print("\nDừng xử lý theo yêu cầu người dùng.")
...
def main():
    ...
    output_path = os.path.join(current_dir, "output", "front_direction_detected.mp4")
    ...
    try:
        ...
        print("Vui lòng chạy room_grid_calibrator_front.py trước để tạo file cấu hình")
        return
    except Exception as e:
